...
% 1. Move the title up (smaller top margin)
\begin{center}
    \vspace*{-1cm} 
    {\huge \textbf{ST3247 SBI Final Report}} \\
    \vspace{0.5cm}
\end{center}
...
Sequential Monte Carlo ABC \citep{sisson2007sequential, beaumont2009adaptive} maintains a population of weighted particles that is propagated through a sequence of decreasing tolerances $\varepsilon_0 > \varepsilon_1 > \dots$ The basic SMC-ABC framework was introduced by \citet{sisson2007sequential}, whose key insight was to apply \emph{partial rejection control} (PRC) to the ABC setting. As most simulator draws at a tight tolerance yield zero weight, PRC replaces those dead particles by resampling and perturbing survivors from the previous generation, ensuring that every particle in the final population carries useful information. We follow the adaptive variant of \citet{beaumont2009adaptive}, which sets each new tolerance as a quantile of the current distances and uses a perturbation kernel whose covariance is $2\widehat{\text{Cov}}$ of the alive particles. At each generation, particles are resampled in proportion to their weights, perturbed by this Gaussian kernel, and accepted at the tighter tolerance. Following \citet{beaumont2009adaptive}, importance weights are recomputed as
\begin{equation}
w_i^{(t)} \propto \frac{\pi(\theta_i^{(t)})}{\sum_j w_j^{(t-1)} K(\theta_i^{(t)} \mid \theta_j^{(t-1)})}.
\end{equation}
The intuition behind these weights is a standard importance-sampling correction. At generation $t$, new particles are not drawn from the prior $\pi$ but from a mixture of Gaussian kernels centred on the previous generation's particles, $\sum_j w_j^{(t-1)} K(\cdot \mid \theta_j^{(t-1)})$. To target the ABC posterior under the prior, each particle must be reweighted by the ratio of the target density (the prior, in the numerator) to the proposal density (the kernel mixture, in the denominator). In practice, this means that particles landing in high-density regions of the previous cloud receive a large denominator and are down-weighted, preventing the population from collapsing onto a narrow region dictated by the previous generation's geometry; conversely, particles that reach prior-supported regions far from the previous cloud are up-weighted, preserving diversity. We use $1{,}000$ particles, $10$ generations, and an adaptive tolerance $\varepsilon_t = $ median of distances at generation $t-1$.
...
Neural Posterior Estimation (NPE) \citep{papamakarios2016fast, greenberg2019automatic} trains a conditional density estimator $q_\phi(\theta \mid S)$ --- here a Masked Autoregressive Flow (MAF) \citep{papamakarios2017masked} --- on the joint $(\theta, S)$ samples produced by the prior simulator. A MAF is a normalising flow that transforms a simple base distribution (a standard Gaussian) into a complex target through a stack of autoregressive affine layers: each layer predicts the scale and shift for dimension~$i$ using only dimensions $1,\dots,i{-}1$, which keeps the Jacobian triangular and makes the density exactly tractable to evaluate. This expressiveness is important here because the $(\beta,\gamma,\rho)$ posterior exhibits strong correlations --- especially the $\beta$--$\rho$ ridge identified in Section~\ref{sec:summaries} --- and non-Gaussian shape that a simple parametric family (e.g.\ a multivariate Gaussian) would miss. Training minimises the negative log-likelihood of $\theta_i$ given $S_i$ over the simulated pairs:
\begin{equation}
\mathcal{L}(\phi) = -\frac{1}{N}\sum_{i=1}^{N} \log q_\phi(\theta_i \mid S_i).
\end{equation}
Once trained, the posterior at the observed summary $S_{\text{obs}}$ can be sampled from in milliseconds without any further simulation. Crucially, NPE is \emph{amortised}: it would also give cheap posteriors for any other observed dataset, without retraining. We use the \texttt{sbi} package \citep{tejero2020sbi} with the default MAF and reuse the same $50{,}000$ simulations from the rejection ABC run.
...
\item \textbf{NPE delivers the tightest posteriors at the lowest cost-per-precision.} It matches synthetic likelihood on $\rho$ (CI width $0.093$ vs.\ $0.098$) and slightly beats it on $\beta$, while running in $60\,$s versus $1{,}363\,$s --- a roughly $23\times$ speedup. Crucially, NPE reuses the very same $50{,}000$ simulations as the rejection ABC baseline, so its true cost over and above rejection ABC is the $\sim 17\,$s of training time.
...
Table~\ref{tab:all_methods} shows that the six methods use very different simulation budgets (from $30{,}000$ to $10^6$). To disentangle statistical efficiency from sheer compute, we compare rejection ABC, SMC-ABC, and NPE each using the same $50{,}000$ simulator calls. Rejection ABC and NPE both operate on the identical set of $50{,}000$ prior simulations from the main rejection ABC run; SMC-ABC is run with a budget cap and stops after the last complete generation within the budget (here, $7$ generations consuming ${\sim}65{,}000$ calls).
...
SMC-ABC (7 gen)         & 0.170 & 0.087 & 0.300 & 0.116 & 0.049 & 0.294 \\
NPE (MAF)               & 0.152 & 0.081 & 0.304 & 0.071 & 0.021 & 0.099 \\
\bottomrule
\end{tabular}
\caption{Budget-matched comparison at ${\sim}50{,}000$ simulator calls. Rejection ABC and NPE use the identical $50{,}000$ simulations from the main run; SMC-ABC consumes ${\sim}65{,}000$ calls due to its iterative resampling. NPE achieves the tightest posteriors by a wide margin.}
...
\caption{Marginal posteriors from the budget-matched comparison (${\sim}50{,}000$ simulator calls each). NPE (purple) is sharply concentrated; SMC-ABC (orange) is moderately tighter than rejection ABC (blue).}
\label{fig:budget_matched}
\end{figure}

At equal budget (Table~\ref{tab:budget_matched}, Figure~\ref{fig:budget_matched}), NPE's $\rho$ CI ($0.099$) is $3.6\times$ narrower than rejection ABC's ($0.352$) and $3.0\times$ narrower than SMC-ABC's ($0.294$). This confirms that NPE's advantage in Table~\ref{tab:all_methods} is genuine statistical efficiency --- it extracts more information per simulation --- rather than an artefact of consuming a larger budget.

\subsection{Robustness to seed and tuning choices}

To verify that our conclusions are not artefacts of a single random seed or a particular hyperparameter setting, we re-ran the full rejection ABC + NPE pipeline with three additional seeds (42, 123, 9999) and varied the NPE training-set size ($N_{\text{train}} \in \{10{,}000,\; 25{,}000,\; 50{,}000\}$). Across seeds, rejection ABC posterior medians shift by less than $0.005$ for all three parameters and CI widths vary with a coefficient of variation below $3.5\%$; NPE is even more stable, with median shifts below $0.003$ and width CV below $2\%$. For the training-set sweep, the NPE posterior is already well-converged at $N_{\text{train}} = 10{,}000$ ($|\text{CI}_\rho| = 0.097$) and changes negligibly when the budget is increased to $25{,}000$ or $50{,}000$ ($|\text{CI}_\rho| = 0.097$ and $0.098$ respectively). This indicates that the MAF has sufficient capacity to learn the posterior mapping from a relatively modest training set, and that the $50{,}000$-simulation budget used throughout this report is more than adequate. In summary, neither the random seed nor the NPE training-set size materially affects the conclusions.

\section{Conclusion}

We have applied six simulation-based inference methods to the problem of estimating $(\beta, \gamma, \rho)$ in an adaptive-network SIR epidemic model. The likelihood is intractable because the network co-evolves with the infection state, but the model is cheap to simulate, making SBI a natural framework. The key findings are as follows:
...
\item \textbf{Synthetic likelihood matches NPE in precision but is ${\sim}23\times$ slower.} It would be the method of choice if we did not trust a neural network on this problem, or if the simulation budget were too small to train one.
