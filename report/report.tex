...
\noindent {\bf Github Repository:} \href{changes}{https://github.com/taggyhan123/st3247_project}
...
Table~\ref{tab:all_methods} shows that the six methods use very different simulation budgets (from $30{,}000$ to $10^6$). To disentangle statistical efficiency from sheer compute, we compare rejection ABC, SMC-ABC, and NPE each using approximately $50{,}000$ simulator calls. SMC-ABC is run with a target budget of 50,000 calls. Because the algorithm requires a complete population of particles to compute the next generation's weights, it stops after completing the generation that crosses the budget.
